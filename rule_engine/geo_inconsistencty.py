...
class GeographicInconsistencyRule(AbstractRule):
    ...
    Activates if the implied speed between the last known location and the
    current transaction location exceeds max_speed_kmh.
    """

    def __init__(self, max_speed_kmh: float = 900.0, weight: float = 1.0):
        super().__init__("Geographic Inconsistency", weight)
        self.max_speed_kmh = max_speed_kmh  # Default: average commercial flight speed
    ...
    def evaluate(self, transaction: Dict[str, Any], context: Dict[str, Any] = None) -> float:
        ...
        # Find the most recent transaction that has location data
        last_trans = None
        ...
        distance_km = self.haversine_distance(
            last_trans['latitude'], last_trans['longitude'],
